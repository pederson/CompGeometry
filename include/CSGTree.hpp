...
// // The CSG is a CSGTree that implements a primitive geometry interface
// // so that it is itself a primitive geometry
// // implementing the exten
// template <typename PrimitiveType>
// class CSG : public CSGTree<PrimitiveType>, public PrimitiveType{
// public:
// 	typedef PrimitiveType				LeafT;
// 	typedef CSGTree<PrimitiveType> 		TreeT;


	

// // THESE IMPLEMENT THE INTERFACE
// 	std::shared_ptr<PrimitiveType> copy() const {return std::make_shared<CSGTree>(*this);};

// 	// returns a bounding box
// 	// - only compiles if the LeafT has a function "get_bounding_box()"
// 	typename BoxTypedef<PrimitiveType>::type get_bounding_box() const{
// 		if (TreeT::m_isleaf) return TreeT::m_leaf->get_bounding_box();
// 		return bounding_box(TreeT::m_ldaughter->get_bounding_box(), TreeT::m_rdaughter->get_bounding_box());
// 	}

// 	// Box<2> get_bounding_box() const {return bounding_box(TreeT::m_ldaughter->get_bounding_box(), TreeT::m_rdaughter->get_bounding_box());};

// 	// Box<2> get_bounding_box() const{return get_bounding_box_impl();};
// 	// Box<3> get_bounding_box() const{return get_bounding_box_impl();};
...
// 	// - only compiles if the LeafT has a function "get_outline(int numpoints)"
// 	std::vector<Hull<2>> get_outline(unsigned int npts) const {
// 		if (TreeT::m_isleaf) return TreeT::m_leaf->get_outline(npts);

// 		std::vector<Hull<2>> oleft = TreeT::m_ldaughter->get_outline(npts);
// 		std::vector<Hull<2>> oright = TreeT::m_rdaughter->get_outline(npts);
		

// 		switch (TreeT::m_op){
// 			case UNION:
// 				for (auto j=0; j<oleft.size(); j++){
// 					bool state0 = TreeT::m_rdaughter->contains_point(*(--oleft[j].points.end()));
// 					for (auto it=oleft[j].points.begin(); it != oleft[j].points.end();){
// 						if (TreeT::m_rdaughter->contains_point(*it)) it = oleft[j].points.erase(it);
// 						else it++;
// 					}
// 				}
...
// 				for (auto j=0; j<oright.size(); j++){
// 					for (auto it=oright[j].points.begin(); it != oright[j].points.end();){
// 						if (TreeT::m_ldaughter->contains_point(*it)) it = oright[j].points.erase(it);
// 						else it++;
// 					}
// 				}
// 				break;
...
// 	template <typename PointType>
// 	bool contains_point_impl(const PointType & pt) const{
// 		if (TreeT::m_isleaf) return TreeT::m_leaf->contains_point(pt);

// 		bool lc = TreeT::m_ldaughter->contains_point(pt);
// 		bool rc = TreeT::m_rdaughter->contains_point(pt);
...
// 	template <typename BoxType>
// 	bool contains_box_impl(const BoxType & bx) const{
// 		if (TreeT::m_isleaf) return TreeT::m_leaf->contains_box(bx);

// 		bool lc = TreeT::m_ldaughter->contains_box(bx);
// 		bool rc = TreeT::m_rdaughter->contains_box(bx);
// 		bool rcoll, lcoll;
...
// 				break;
// 			case DIFFERENCE:
// 				rcoll = TreeT::m_rdaughter->collides_box(bx);
// 				return lc && !rcoll;
// 				break;
// 			case XOR:
// 				lcoll = TreeT::m_ldaughter->collides_box(bx);
// 				rcoll = TreeT::m_rdaughter->collides_box(bx);
// 				return (lc || rc) && !(lc && rc) && !lcoll && !rcoll;
// 				break;
// 		}
...
// 	template <typename BoxType>
// 	bool collides_box_impl(const BoxType & bx) const{
// 		if (TreeT::m_isleaf) return TreeT::m_leaf->collides_box(bx);

// 		bool lc = TreeT::m_ldaughter->collides_box(bx);
// 		bool rc = TreeT::m_rdaughter->collides_box(bx);
// 		bool lcont, rcont;
...
// 				break;
// 			case DIFFERENCE:
// 				rcont = TreeT::m_rdaughter->contains_box(bx);
// 				return lc && !rcont;
// 				break;
// 			case XOR:
// 				lcont = TreeT::m_ldaughter->contains_box(bx);
// 				rcont = TreeT::m_rdaughter->contains_box(bx);
// 				return (lc || rc) && !(lcont && rcont);
// 				break;
// 		}
